...
def solve():
    ...
    def F2L(): # questo è l'F2L più brutto della storia :)
        ...
        while (not (WHITE[0] == WHITE[1] == WHITE[2] and GREEN[2][0] == GREEN[2][1] == GREEN[2][2]
                and ORANGE[2][0] == ORANGE[2][1] == ORANGE[2][2] and BLUE[2][0] == BLUE[2][1] == BLUE[2][2])):
    ...
    def cross():
        while (not (WHITE[0][1] == WHITE[1][0] == WHITE[1][2] == WHITE[2][1] == WHITE[1][1]
                and GREEN[2][1] == GREEN[1][1] and ORANGE[2][1] == ORANGE[1][1] and BLUE[2][1] == BLUE[1][1])):
            if (YELLOW[0][1] == WHITE[1][1] or YELLOW[1][0] == WHITE[1][1] or YELLOW[2][1] == WHITE[1][1] or YELLOW[1][2] == WHITE[1][1]): # top - easy
                while (YELLOW[2][1] != WHITE[1][1]): U()
                while (GREEN[0][1] != GREEN[1][1]): d()
                F(); F()
            elif (GREEN[0][1] == WHITE[1][1] or ORANGE[0][1] == WHITE[1][1] or BLUE[0][1] == WHITE[1][1] or RED[0][1] == WHITE[1][1]): # top - adv
                while (ORANGE[0][1] != WHITE[1][1]): U()
                count = 0
                while (YELLOW[1][2] != GREEN[1][1]):
                    count += 1
                    d()
                R1(); F(); R()
            elif (GREEN[1][0] == WHITE[1][1] or GREEN[1][2] == WHITE[1][1] or ORANGE[1][0] == WHITE[1][1] or ORANGE[1][2] == WHITE[1][1]
                  or BLUE[1][0] == WHITE[1][1] or BLUE[1][2] == WHITE[1][1] or RED[1][0] == WHITE[1][1] or RED[1][2] == WHITE[1][1]): # middle
                count = 0
                while (GREEN[1][2] != WHITE[1][1] and ORANGE[1][0] != WHITE[1][1]):
                    count += 1
                    E()
                if (GREEN[1][2] == WHITE[1][1]):
                    R(); U(); R1()
                else:
                    F1(); U1(); F()
                for _ in range(count): E()
            elif (GREEN[2][1] == WHITE[1][1] or ORANGE[2][1] == WHITE[1][1] or BLUE[2][1] == WHITE[1][1] or RED[2][1] == WHITE[1][1]): # bottom
                count = 0
                while (GREEN[2][1] != WHITE[1][1]):
                    count += 1
                    d()
                F1()
                for _ in range(count): E()
    ...
    for i in range(len(MOVES) - 3): # elimino 4 mosse uguali
        if ( MOVES[i] != "&" and MOVES[i] == MOVES[i + 1] == MOVES[i + 2] == MOVES[i + 3]):
            MOVES[i] = "&"
            MOVES[i + 1] = "&"
            MOVES[i + 2] = "&"
            MOVES[i + 3] = "&"
    for i in range(len(MOVES) - 2): # scambio tre mosse uguali con l'opposta
        if ( MOVES[i] != "&" and MOVES[i] == MOVES[i + 1] == MOVES[i + 2]):
            ...
            MOVES[i + 1] = "&"
            MOVES[i + 2] = "&"
    for i in range(len(MOVES) - 1): # elimino le coppie inutili (es. R R')
...
def scramble():
    ...
    with open(r"C:\Users\aless\OneDrive\Python\ProjectZ\scrambles.txt", "w") as f:
        f.writelines(lines)
    return ("L U L U D1 R D1 L1 F1 R U1 F1 D R1 U1 L1 B L B R1 U B D R1 B1")
